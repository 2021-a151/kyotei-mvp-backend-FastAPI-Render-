...
def ingest(date: str, api_key: str | None = None, db: Session = Depends(get_db)):
    ...
    - dateはYYYYMMDD文字列を想定（例：20260201）
    """
    _require_admin(api_key)

    # ダミーのrace_id（場=11, R=1固定）
    ...
    db.execute(text("""
      ...
      values (:race_id, 'GO', '[]'::jsonb, '["TIME_TOP3","ST_TOP3"]'::jsonb,
    ...
    """), {
        ...
        "details": '[{"key":"showTimeRank","label":"展示タイム","value":"2位"},{"key":"showStRank","label":"展示ST","value":"3位"}]',
        "now": now
    })

    # picks UPSERT（最大7点ダミー）
    db.execute(text("""
      insert into public.picks (race_id, max_points, tickets, computed_at)
      values (:race_id, 7, :tickets::jsonb, :now)
      on conflict (race_id) do update set
        tickets = excluded.tickets,
        computed_at = excluded.computed_at
    """), {
        "race_id": race_id,
        "tickets": '[{"ticket":"1-2-6","rank":1,"tag":"RECOMMENDED"},{"ticket":"1-6-2","rank":2,"tag":"RECOMMENDED"},{"ticket":"1-2-3","rank":3,"tag":"RECOMMENDED"},{"ticket":"1-3-2","rank":4,"tag":"RECOMMENDED"},{"ticket":"1-2-4","rank":5,"tag":"RECOMMENDED"},{"ticket":"1-4-2","rank":6,"tag":"RECOMMENDED"},{"ticket":"1-2-5","rank":7,"tag":"RECOMMENDED"}]',
        "now": now
    })
